...
def cnn_model_fn(features, labels, mode):
    ...
    dropout = tf.keras.layers.Dropout(rate=0.4)(
        dense, training=mode == tf.estimator.ModeKeys.TRAIN)
...
def get_input_fn(data, labels, batch_size):
    """Generates the input function for the model returned
    by generate_model()."""

    return te.inputs.numpy_input_fn(
        x={"x": data},
        y=labels,
