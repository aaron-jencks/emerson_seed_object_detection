...
from cam_viewer.cam_util.cam_ctrl_util import PMDCam, RealsenseCam
...
class CamCtrl(ThreadedSocketedStateMachine):
    ...
    def find_cam(self):
        ...
        self.cam = PMDCam(disp_parent=self.parent) if self.pmd else RealsenseCam(disp_parent=self.parent)
        self.configure_cam()
        self.cam.connect()
